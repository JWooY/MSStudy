...
namespace review
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            {
                Program.PrintCharacterByCharacter("모든것이 두렵다. 저 벽 뒤의 존재가 나를 해칠지도 모르지 않는가?");
                Console.ReadLine();
                Console.Clear();
            ...
            Program.PrintCharacterByCharacter("그 앞엔 어떤 작은 키의 여자가 놀란 표정으로 서있었다.");
            Console.ReadLine();
            Console.Clear();
            Program.PrintCharacterByCharacter("\"으앗! 놀래라! 뭐야 진짜 누가 있잖아?\"");
            Console.ReadLine();
            Console.Clear();
            Program.PrintCharacterByCharacter("쏟아져나오려던 비명을 간신히 집어삼키며 물었다.");
            Console.ReadLine();
            Console.Clear();
            Program.PrintCharacterByCharacter("\"누, 누구세요?\"");
            Console.ReadLine();
            Program.PrintCharacterByCharacter("\"그걸 나한테 물으면 어쩌자는 거에요?\"");
            Console.ReadLine();
            Program.PrintCharacterByCharacter("\"그쪽이 저를 가둔 거 아니예요?\"");
            Console.ReadLine();
            Program.PrintCharacterByCharacter("\"그건 제가 할 말이거든요?\"");
            Console.ReadLine();
            Console.Clear();
            Program.PrintCharacterByCharacter("어이가 없다는 듯 헛웃음 치던 그녀는 이내 심각한 표정이 되었다.");
            Console.ReadLine();
            Console.Clear();
            Program.PrintCharacterByCharacter("\"당신도 아니라면...\"");
            Console.ReadLine();
            Console.Clear();
            Program.PrintCharacterByCharacter("그녀는 잠시 말을 멈추고, 눈을 감았다가 다시 뜨며 말을 이었다.");
            Console.ReadLine();
            Console.Clear();
            Program.PrintCharacterByCharacter("\"사실... 저도 갇힌 지 얼마 안 됐어요.\"");
            Console.ReadLine();
            Console.Clear();
            Program.PrintCharacterByCharacter("그녀는 낮은 목소리로 말했다..");
            Console.ReadLine();
            Program.PrintCharacterByCharacter("그녀의 말에 나는 잠시 침묵했다. 그럼, 여긴 대체...?");
            Console.ReadLine();
            Program.PrintCharacterByCharacter("그때, 나는 그녀의 어깨너머로 보이는 방 침대 아래에서 뭔가 이상한 것을 발견했다.");
            Console.ReadLine();
            Program.PrintCharacterByCharacter("침대 아래를 살펴보니, 사람 하나 겨우 들어갈 만한 작은 문이 있었다.");
            Console.ReadLine();
            Program.PrintCharacterByCharacter("나는 조심스럽게 그 문을 가리켰다.");
            Console.ReadLine();
            Console.Clear();
            Program.PrintCharacterByCharacter("\"어? 저기... 저 문 좀 보세요. 혹시 저걸 통해 나갈 수 있을지도 몰라요.\"");
            Console.ReadLine();
            Console.Clear();
            Program.PrintCharacterByCharacter("그녀는 내 손끝을 따라 문을 보았다. 그녀는 깜짝놀라 말했다");
            Console.ReadLine();
            Console.Clear();
            Program.PrintCharacterByCharacter("\"이, 이런곳에 문이?\"");
            Console.ReadLine();
            Console.Clear();
            Program.PrintCharacterByCharacter("나는 얼른 침대를 옆으로 밀어내곤 다락문처럼 생긴 낡은 나무 문을 바라보았다.");
            Console.ReadLine();
            Console.Clear();
            Program.PrintCharacterByCharacter("\"뭐가 뭔지는 모르겠지만... 알아낼 방법은 이거 하나뿐인 것 같네요\"");
            Console.ReadLine();
            Program.PrintCharacterByCharacter("\"그, 그렇네요 그럼...\"");
            Console.ReadLine();
            Console.Clear();
            Program.PrintCharacterByCharacter("나는 조심스럽게 문을 열어보았다.");
            Console.ReadLine();
            Console.Clear();
            Program.PrintCharacterByCharacter("남은 시간 23:00:00");
            Console.ReadLine();
            Console.Clear();

            Program.PrintCharacterByCharacter("갑작스럽게 아까전 울려퍼진 목소리가 들려왔다. 순간 놀라서 몸을 흠칫 떨었다");
            Console.ReadLine();
            Console.Clear();
            Program.PrintCharacterByCharacter("\"뭐예요? 무슨 문제 있어요?\"");
            Console.ReadLine();
            Console.Clear();
            Program.PrintCharacterByCharacter("어리둥절한 표정으로 바라보는 그녀를 보며 생각했다.");
            Console.ReadLine();
            Program.PrintCharacterByCharacter("그녀에겐 이 목소리가 들리지 않는건가?");
            Console.ReadLine();
            Console.Clear();
            Program.PrintCharacterByCharacter("\"안갈거예요? 무서운건가? 참나, 덩치는 산만해가지고... 그럼 제가 먼저 가죠\"");
            Console.ReadLine();
            Console.Clear();
            Program.PrintCharacterByCharacter("멈춰선 나를 답답해 하던 그녀가 문을 여는 순간, 어두운 좁은 통로가 펼쳐졌다. 통로는 온통 어둠뿐이었다.");
            Console.ReadLine();
            Program.PrintCharacterByCharacter("그녀는 잠시 망설였지만, 결국 한 발자국씩 그 어두운 통로로 들어갔다.");
            Console.ReadLine();
